...
plt.subplot(2,1,1)
plt.plot(t[ep[pos]+1:ep[pos+1]+1], Y, 'g', label=r'$\theta$')
plt.legend(bbox_to_anchor=(.875, .785), loc='center')
plt.ylabel(r'$\theta$ (rad)')
plt.title('Maior episódio')
plt.ylim(-.3, .5)
plt.subplot(2,1,2)
plt.plot(t[ep[pos]+1:ep[pos+1]+1], U, 'b')
plt.xlabel('Amostras')
plt.ylabel('Esquerda $\Leftarrow$ u $\Rightarrow$ Direita')
plt.ylim(-.1, 1.85)
plt.show()